...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _update_teacher(self) -> None:
        ...
        if not self_distillation_cfg or loss_mode not in ("score", "decode"):
            return
    ...
    def update_policy(self, data: DataProto):
        ...
        # ---- Inline SFT mode: simple cross-entropy on successful solutions ----
        if data.meta_info.get("sft_mode", False):
            return self._sft_update(data)

        temperature = data.meta_info["temperature"]  # temperature must be in the data.meta_info to avoid silent error
        pad_token_id = data.meta_info.get("pad_token_id", 0)
        # Support dynamic loss_mode from two-phase training (via meta_info)
        loss_mode = data.meta_info.get("loss_mode", self.config.policy_loss.get("loss_mode", "vanilla"))

        self_distillation_enabled = loss_mode in ("score", "decode")
        is_decode_mode = loss_mode == "decode"
        self_distillation_cfg = getattr(self.config, "self_distillation", None)
        if self_distillation_enabled:
            ...
            if "error_types" in data.batch:
                self_distillation_required_keys.add("error_types")
        ...
        # Include critique training data for auxiliary critique loss
        critique_loss_beta = self_distillation_cfg.get("critique_loss_beta", 0.0) if self_distillation_cfg else 0.0
        critique_keys = ["critique_input_ids", "critique_attention_mask", "critique_position_ids", "critique_response_mask"]
        has_critique_data = all(k in data.batch.keys() for k in critique_keys)
        if critique_loss_beta > 0 and has_critique_data:
            select_keys.extend(critique_keys)
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self_distillation_enabled:
                        ...
                        if is_decode_mode:
                            from verl.trainer.ppo.core_algos import compute_tpdd_loss
                            pg_loss, pg_metrics = compute_tpdd_loss(
                                student_log_probs=log_prob,
                                teacher_log_probs=teacher_log_prob,
                                student_topk_log_probs=student_topk_logps,
                                teacher_topk_log_probs=teacher_topk_logps,
                                student_topk_indices=student_topk_indices,
                                response_tokens=model_inputs["responses"],
                                response_mask=response_mask,
                                error_types=model_inputs["error_types"],
                    ...
                    # --- Auxiliary critique loss: train model to predict pass/fail ---
                    if critique_loss_beta > 0 and has_critique_data:
                        crit_ids = model_inputs.get("critique_input_ids")
                        crit_mask = model_inputs.get("critique_attention_mask")
                        crit_pos = model_inputs.get("critique_position_ids")
                        crit_resp_mask = model_inputs.get("critique_response_mask")
                        if crit_ids is not None and crit_resp_mask.sum() > 0:
                            with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
                                crit_output = self.actor_module(
                                    input_ids=crit_ids,
                                    attention_mask=crit_mask,
                                    position_ids=crit_pos,
                                    use_cache=False,
                                )
                            crit_logits = crit_output.logits  # (bsz, seq_len, vocab)
                            # Shift: predict next token from position i using logits at position i
                            shift_logits = crit_logits[:, :-1, :].float()
                            shift_labels = crit_ids[:, 1:]
                            shift_mask = crit_resp_mask[:, 1:]
                            # Per-token NLL
                            crit_log_probs = logprobs_from_logits(shift_logits, shift_labels)
                            critique_loss = -(crit_log_probs * shift_mask).sum() / shift_mask.sum().clamp(min=1)
                            policy_loss = policy_loss + critique_loss_beta * critique_loss
                            micro_batch_metrics["actor/critique_loss"] = critique_loss.detach().item()
                            micro_batch_metrics["actor/critique_loss_beta"] = critique_loss_beta
    ...
    def _sft_update(self, data: DataProto) -> dict:
        """Inline SFT: cross-entropy loss on successful solutions."""
        temperature = data.meta_info.get("temperature", 1.0)
        pad_token_id = data.meta_info.get("pad_token_id", 0)
        sft_steps = data.meta_info.get("sft_steps", 2)
        metrics = defaultdict(lambda: 0.0)

        total_samples = data.batch.batch_size[0]
        # Move all tensors to the correct device (they were stored on CPU in the buffer)
        data = data.to(get_device_id())
        print(f"\n{'='*60}", flush=True)
        print(f"  [SFT] Starting: {total_samples} samples, {sft_steps} steps", flush=True)
        ...
        sft_pbar = tqdm(range(sft_steps), desc="[SFT] Training", leave=False)
        for step_i in sft_pbar:
            self.actor_optimizer.zero_grad()
            step_loss = 0.0
            for mb_i, micro_batch in enumerate(micro_batches):
                ...
                log_prob = outputs["log_probs"]
                sft_loss = -(log_prob * response_mask).sum() / response_mask.sum().clamp(min=1)
                loss = sft_loss / n_micro
                loss.backward()
                step_loss += sft_loss.detach().item() / n_micro

            grad_norm = self._optimizer_step()
            metrics["sft/loss"] = step_loss
            metrics["sft/grad_norm"] = grad_norm.detach().item()
            sft_pbar.set_postfix(loss=f"{step_loss:.4f}", grad_norm=f"{grad_norm:.4f}")
