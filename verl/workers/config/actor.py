...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        Distillation is enabled when policy_loss.loss_mode == "score".
    ...
    # TPDD (Decode) parameters
    tpdd_beta: float = 1.0
    tpdd_gamma: float = 1.0
    enriched_reprompt_template: Optional[str] = None
    critique_loss_beta: float = 0.0
...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options: 'vanilla', 'clip-cov', 'kl-cov', 'gpg', 'score'.
