...
# =============================================================================
# TPDD: Tri-Partite Decoupled Distillation (Decode)
# =============================================================================

TPDD_ERROR_WEIGHTS = {
    0: (0.0, 0.0),   # Passed
    1: (1.5, 0.3),   # Runtime  – high locality
    2: (1.0, 1.0),   # Wrong Answer – mixed
    3: (0.3, 1.5),   # Timeout  – low locality
    4: (1.5, 0.3),   # Compile  – high locality
}


def compute_tpdd_loss(
    student_log_probs: torch.Tensor,
    teacher_log_probs: torch.Tensor,
    ...
    """Compute TPDD loss: TCK + beta*w_ECK*ECK + gamma*w_NCK*NCK with error-type-aware weights."""
    metrics = {}
    bs, resp_len, K = student_topk_log_probs.shape
    ...
    # k_T: teacher argmax (within student's top-k indices)
    teacher_argmax_in_topk = teacher_topk_log_probs.argmax(dim=-1)  # (bs, resp_len)
    k_T = student_topk_indices.gather(-1, teacher_argmax_in_topk.unsqueeze(-1)).squeeze(-1)

    # k_S: student sampled token
    k_S = response_tokens
    ...
    # ---- TCK: -log p_S(k_T) ----
    k_T_expanded = k_T.unsqueeze(-1).expand_as(student_topk_indices)
    k_T_match = (student_topk_indices == k_T_expanded)
    k_T_in_topk = k_T_match.any(dim=-1)
    k_T_topk_idx = k_T_match.float().argmax(dim=-1)
    student_log_prob_at_k_T = student_topk_log_probs.gather(-1, k_T_topk_idx.unsqueeze(-1)).squeeze(-1)
    fallback_logp = torch.full_like(student_log_prob_at_k_T, -12.0)
    student_log_prob_at_k_T = torch.where(k_T_in_topk, student_log_prob_at_k_T, fallback_logp)
    tck_loss = -student_log_prob_at_k_T

    # ---- ECK: -M_t * log(1 - p_S(k_S)) ----
    p_S_at_k_S = student_log_probs.exp().clamp(max=1.0 - 1e-7)
    eck_loss = -M_t * torch.log(1.0 - p_S_at_k_S)

    # ---- NCK: KL(p_T_hat || p_S_hat) on residual distribution ----
    k_S_expanded = k_S.unsqueeze(-1).expand_as(student_topk_indices)
    exclude_mask = k_T_match | (student_topk_indices == k_S_expanded)
    NEG_INF = -1e10
    student_residual = student_topk_log_probs.masked_fill(exclude_mask, NEG_INF)
    teacher_residual = teacher_topk_log_probs.masked_fill(exclude_mask, NEG_INF)
    ...
    student_residual = add_tail(student_residual)
    teacher_residual = add_tail(teacher_residual)
    student_residual = student_residual - torch.logsumexp(student_residual, dim=-1, keepdim=True)
    teacher_residual = teacher_residual - torch.logsumexp(teacher_residual, dim=-1, keepdim=True)
    nck_loss = F.kl_div(student_residual, teacher_residual, reduction="none", log_target=True).sum(-1)

    # ---- Error-type-aware weighting ----
    beta = getattr(self_distillation_config, "tpdd_beta", 1.0)
    gamma = getattr(self_distillation_config, "tpdd_gamma", 1.0)

    w_eck = torch.zeros(bs, device=device)
    w_nck = torch.zeros(bs, device=device)
    for etype, (we, wn) in TPDD_ERROR_WEIGHTS.items():
        mask_e = (error_types == etype)
        w_eck[mask_e] = we
        w_nck[mask_e] = wn

    per_token_loss = tck_loss + beta * w_eck.unsqueeze(1) * eck_loss + gamma * w_nck.unsqueeze(1) * nck_loss

    # ---- IS clipping ----
    is_clip = self_distillation_config.is_clip
    if is_clip is not None:
        if old_log_probs is None:
            raise ValueError("old_log_probs is required for TPDD IS ratio.")
        negative_approx_kl = (student_log_probs - old_log_probs).detach()
        negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
        ratio = torch.exp(negative_approx_kl).clamp(max=is_clip)
        per_token_loss = per_token_loss * ratio
    ...
    with torch.no_grad():
        ...
        metrics["tpdd/tck_loss"] = (tck_loss * loss_mask).sum().item() / valid.item()
        metrics["tpdd/eck_loss"] = (eck_loss * loss_mask).sum().item() / valid.item()
        metrics["tpdd/nck_loss"] = (nck_loss * loss_mask).sum().item() / valid.item()
        metrics["tpdd/mismatch_rate"] = (M_t * loss_mask).sum().item() / valid.item()
        for etype in range(5):
            metrics[f"tpdd/error_type_{etype}_frac"] = (error_types == etype).float().mean().item()
        T = resp_len
        q1, q3 = T // 4, (3 * T) // 4
        eck_total = (eck_loss * loss_mask).sum().clamp(min=1e-8)
        metrics["tpdd/eck_head_ratio"] = ((eck_loss[:, :q1] * loss_mask[:, :q1]).sum() / eck_total).item()
        metrics["tpdd/eck_tail_ratio"] = ((eck_loss[:, q3:] * loss_mask[:, q3:]).sum() / eck_total).item()
