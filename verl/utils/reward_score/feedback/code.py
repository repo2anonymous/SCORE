...
# Error type constants for TPDD (Tri-Partite Decoupled Distillation)
ERROR_TYPE_PASSED = 0
ERROR_TYPE_RUNTIME = 1
ERROR_TYPE_WRONG_ANSWER = 2
ERROR_TYPE_TIMEOUT = 3
ERROR_TYPE_COMPILE = 4

_COMPILE_KEYWORDS = ("SyntaxError", "IndentationError", "ImportError", "NameError")
_RUNTIME_KEYWORDS = (
    "IndexError", "TypeError", "ValueError", "KeyError", "AttributeError",
    "ZeroDivisionError", "OverflowError", "RecursionError", "MemoryError",
    "AssertionError", "StopIteration", "Traceback",
)
_TIMEOUT_KEYWORDS = ("Time Limit Exceeded", "timeout", "timed out", "Time out")


def classify_error_type(feedback: str, accuracy: float) -> int:
    """Classify error type from execution feedback. Priority: passed > compile > timeout > runtime > wrong_answer."""
    if accuracy == 1.0:
        return ERROR_TYPE_PASSED
    if any(kw in feedback for kw in _COMPILE_KEYWORDS):
        return ERROR_TYPE_COMPILE
    if any(kw in feedback for kw in _TIMEOUT_KEYWORDS):
        return ERROR_TYPE_TIMEOUT
    if any(kw in feedback for kw in _RUNTIME_KEYWORDS):
        return ERROR_TYPE_RUNTIME
    return ERROR_TYPE_WRONG_ANSWER


ERROR_TYPE_LABELS = {
    ERROR_TYPE_PASSED: "passed",
    ERROR_TYPE_RUNTIME: "runtime_error",
    ERROR_TYPE_WRONG_ANSWER: "wrong_answer",
    ERROR_TYPE_TIMEOUT: "timeout",
    ERROR_TYPE_COMPILE: "compile_error",
}
...
def compute_score(solution: str, ground_truth: str, extra_info = None, sparse_rewards=False, max_test_cases=None):
    ...
    try:
        ...
        return {
            ...
            "error_type": ERROR_TYPE_COMPILE,
            "error_type_label": ERROR_TYPE_LABELS[ERROR_TYPE_COMPILE],
    ...
    if sparse_rewards:
        ...
        # Hybrid reward: full pass gets big bonus, partial pass gets scaled-down signal
        # This prevents the model from "cheating" by only passing easy test cases
        if accuracy == 1.0:
            reward = 1.0
        else:
            reward = accuracy * 0.3
    ...
    feedback_str = format_test_feedback(records, was_truncated=was_truncated)
    error_type = classify_error_type(feedback_str, accuracy)
    return {
        "score": reward,
        "acc": accuracy,
        "pred": predictions,
        ...
        "error_type_label": ERROR_TYPE_LABELS[error_type],
